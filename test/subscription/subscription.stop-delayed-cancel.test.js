...
describe('Subscription', () => {
	...
	before(async () => {
		...
		// [delay-cancel] a subscripion to [stop-delay-cancel] with
		await login(user.email, user.rawPass);
		options.body = {
	...
	describe('Stop Delayed Cancel', () => {
		beforeEach(async () => {
			options.transform = null;
			options.body = {};
			options.method = 'POST';
			options.uri = signPath('/subscription/stop-delayed-cancel', 'POST');
		});
		...
		it('Should be successful and return proper data', async () => {
			...
			expect(data.delayed_cancel_at).to.equal(null); // delayed_cancel_at should be null
		});

		/* Negative tests */
		it('Should not be able to [stop-delay-cancel] a subscription that is [cancelled]', async () => {
			await login(userB.email, userB.rawPass);
			options.body = {
				cancellation_message: 'Switching providers',
				reason_code: 'TEST',
		...
		it('Should not be able to [stop delay-cancel] with a user who has no subscriptions', async () => {
			await login(userC.email, userC.rawPass);
			await checkStatusCodeByOptions(options, 400);
		});

		it('Should not be able to [stop-delay-cancel] a [trial] subscription', async () => {
			await login(userC.email, userC.rawPass);
			await checkStatusCodeByOptions(options, 400);
		});
	});
