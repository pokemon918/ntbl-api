...
describe('Images', () => {
	describe('Get Tasting Image', () => {
		...
		before(async () => {
			...
			var tastingResponse = await createItem(options.uri, options.body);

			// Upload Tasting Image
			...
			var uploadOptions = {
				method: 'POST',
				uri: signPath('/tasting/addImage/' + tastingResponse.data.ref, 'POST'),
				formData: {
					name: makeUniqueString(),
					uploadedFile: {
						value: file,
