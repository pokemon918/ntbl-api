...
describe('Contest', () => {
	...
	describe('Contest Team making a statement about an impression', () => {
		beforeEach(async () => {
			options.transform = null;
			options.method = 'POST';
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${contestCollection.ref}/subject/${contestImpression.ref}/statement`,
				'POST'
			);
		...
		it('should be successful and return proper data upon insert if user is [contest admin]', async () => {
			...
			expect(updatedContest.data.statements.length).to.equal(1);
			let responseStatement = updatedContest.data.statements[0];
		...
		it('should not be able to make a statement using a traditional team (as contest)', async () => {
			...
			options.uri = signPath(
				`/contest/${traditionalTeam.data.ref}/collection/${contestCollection.ref}/subject/${contestImpression.ref}/statement`,
				'POST'
			);
		...
		it('should not be able to make a statement using a traditional collection', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${traditionalCollection.data.ref}/subject/${contestImpression.ref}/statement`,
				'POST'
			);
			await checkStatusCodeByOptions(options, 400);
		});
		...
		it('should not be able to make a statement using a traditional impression', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${contestCollection.ref}/subject/${traditionalImpression.data.ref}/statement`,
				'POST'
			);
			await checkStatusCodeByOptions(options, 400);
		});
		...
		it('should not be able to make a statement using a invalid team (as contest)', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${makeUniqueString()}/collection/${contestCollection.ref}/subject/${
		...
		it('should return an error if the [sub entities] does not belong to the contest', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${anotherContestTeam.data.ref}/collection/${contestCollection.ref}/subject/${contestImpression.ref}/statement`,
				'POST'
			);
			await checkStatusCodeByOptions(options, 400);
		});
		...
		it('should return an error if the [collection] does not belong to the contest', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${anotherContestCollection.ref}/subject/${contestImpression.ref}/statement`,
				'POST'
			);
			await checkStatusCodeByOptions(options, 400);
		});
		...
		it('should return an error if the [impression] does not belong to the collection', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${contestCollection.ref}/subject/${anotherContestImpression.ref}/statement`,
				'POST'
			);
			await checkStatusCodeByOptions(options, 400);
		});
		...
		it('should return an error if the user is a [leader]', async () => {
			await login(divisionLeader.email, divisionLeader.rawPass);
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${contestCollection.ref}/subject/${contestImpression.ref}/statement`,
				'POST'
		...
		it('should return an error if the [subject] is from another contest', async () => {
			await login(anotherContestDivisionMember.email, anotherContestDivisionMember.rawPass);
			options.uri = signPath(
				`/contest/${contestTeam.data.ref}/collection/${contestCollection.ref}/subject/${anotherContestCollection.ref}/statement`,
				'POST'
			);
			await checkStatusCodeByOptions(options, 400);
		});
