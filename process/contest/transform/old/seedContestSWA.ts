...
async function init() {
	...
	let filePath = np.join(__dirname, './data-swa/collection_a_impressions.json');
	...
	filePath = np.join(__dirname, './data-swa/collection_b_impressions.json');
	...
	filePath = np.join(__dirname, './data-swa/collection_c_impressions.json');
	fileName = np.basename(filePath);
	type = mime.contentType(fileName);
	file = fs.createReadStream(filePath);
